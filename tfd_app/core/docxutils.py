...
def detect_heading(text):
    ...
    if re.match(r"^\d{1,3}(?:\.\d{1,3})+", t):
        ...
        if m:
            ...
            # 量级护栏（v1.3.125）：标题层级编号各段均为小整数（章≤50、节/子节≤99）。
...
def _looks_like_body_despite_heading_style(text):
    ...
    if not t:
        return False
    # 强标题前缀豁免（即便超长或带句末标点）——避免真实长章标题被误降级。
    # 以"第X章/节/篇/编"或多级编号(1.1/2.3.1) 或中文序数(一、二、)开头的段落，
    # 默认判为标题而非正文。但"第X章…"后接完整分析/论证句（含"通过…分析/得出/
    # 因此/从而/综上所述/本文认为"等论证性短语），实为章节内容概述而非标题，
    # 不应套 Heading 样式（例："第三章通过研究分析H证券公司…外部环境，得出现阶段…"）。
    ...
    # 枚举列举项：数字+.+紧接着中文/字母（如「1.利用牌照优势…」「2.增加有价值的…」）
    # 是正文列举，绝非标题。数字后无空格、非多级编号（1.1）、直接接正文内容的，应降级为正文，
